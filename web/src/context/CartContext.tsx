  ...
  const applyServerCart = (serverCart: Awaited<ReturnType<typeof cartApi.getCart>>) => {
    ...
    const mappedItems: CartItem[] = serverCart.items.map((item: any) => ({
      ...
      product: item.product || {
        id: item.productId,
        title: item.productName || item.productTitle || item.name || 'Botanical Plant',
        name: item.productName || item.productTitle || item.name || 'Botanical Plant',
      ...
      variant: item.variant || {
        ...
        sku: item.sku || '',
        priceAdjustment: 0,
        stockQuantity: item.availableStock ?? 100,
        lowStockThreshold: 5,
        isDefault: true,
        isActive: true,
      },
